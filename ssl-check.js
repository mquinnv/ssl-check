      ...
      monitors.forEach(function(monitor) {
        if (monitor.url.indexOf('http://') === 0) {
          ...
          https.get('https' + monitor.url.substring(4), function(res) {
            ...
            if (
              !monitor.valid_to ||
              valid_to.getTime() !== monitor.valid_to.getTime()
            ) {
              console.log(
                'updating expiration',
              ...
              db.update(
                {_id: monitor.id},
                {$set: {valid_to: new Date(cert.valid_to)}},
              );
            }
          });
        }
...
prog
  .command('insert')
  .description(
    'get the list of checks from UptimeRobot and insert any missing ones into local db',
  )
  .action(function() {
...
prog
  .command('recheck')
  .description(
    'recheck each expired or expiring cert and notify slack on renewal',
  )
  .action(function() {
    var today = moment();
    db.find(
          ...
          https.get('https' + monitor.url.substring(4), function(res) {
            ...
            if (
              ...
              hook({
                msg:
                  '<https' +
                  monitor.url.substring(4) +
                  '|' +
                  monitor.friendlyname +
                  '> until ' +
                  moment(valid_to).format('D-MMM-YYYY'),
                title: 'Renewal',
                color: 'good',
              });
            } else {
    ...
    db.find(
      ...
      function(err, monitors) {
        var split = _(monitors)
          .sortBy('valid_to')
          .partition(function(monitor) {
            return moment(monitor.valid_to).isAfter(today);
          })
          .value();
        function msgify(monitors) {
          return _.reduce(
            _.sortBy(monitors, 'valid_to'),
            function(msg, monitor) {
              return (
                ...
                monitor.friendlyname +
                '> ' +
                moment(monitor.valid_to).fromNow()
              );
            },
            '',
          );
